                ...
                return (
                  <div key={w.strategy} style={{ padding: "10px 0", borderTop: `1px solid ${COLORS.border}` }}>
                    <div className="flex items-center justify-between" style={{ fontSize: "0.8rem" }}>
                      <span className="flex items-center" style={{ gap: 6, color: "var(--parchment)" }}>
                        {stratName}
                        {w.relative != null && (
                          <span style={{ fontSize: "0.65rem", color: w.relative >= 0 ? "var(--cold-bright)" : "var(--ember)" }}>
                            {w.relative >= 0 ? "▲" : "▼"}
                          </span>
                        )}
                      </span>
                    ...
                    {w.confidence != null && (
                      <div style={{ marginTop: 6, height: 3, borderRadius: 999, background: "rgba(44,36,56,0.6)", overflow: "hidden" }}>
                        <div style={{ height: "100%", width: `${Math.round(w.confidence * 100)}%`, borderRadius: 999, background: "linear-gradient(90deg, var(--cold), var(--gold-bright))" }} />
                      </div>
                    )}
                  </div>
                );
...
function PredictionsView({ setView }) {
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([
      supabase.from("predictions").select("target_draw_date, rank, back2, front3, back3, first_prize, agreement_score, contributing_strategies, source, generated_at").order("target_draw_date", { ascending: false }).limit(60),
      supabase.from("prediction_evaluations").select("target_draw_date, rank, source, back2_exact_hit, front3_any_hit, back3_any_hit, actual_back2"),
    ]).then(([predRes, evalRes]) => {
      if (cancelled) return;
      const evalMap = new Map();
      if (!evalRes.error && evalRes.data) {
        for (const e of evalRes.data) evalMap.set(`${e.target_draw_date}|${e.rank}|${e.source}`, e);
      }
      if (!predRes.error && predRes.data) {
        setRows(predRes.data.map((p) => ({ ...p, evaluation: evalMap.get(`${p.target_draw_date}|${p.rank}|${p.source}`) ?? null })));
      }
      setLoading(false);
    });
    return () => { cancelled = true; };
  ...
  const back2Hits = evaluated.filter((r) => r.evaluation.back2_exact_hit).length;

  return (
    <div className="mx-auto flex flex-col" style={{ maxWidth: 950, gap: 20 }}>
      <div>
        <p className="ck-eyebrow" style={{ marginBottom: 4 }}>ประวัติคำทำนาย</p>
        <h1 className="ck-display" style={{ fontSize: "1.6rem", fontWeight: 600, color: "var(--parchment)" }}>คำทำนายจริงทั้งหมดจาก Supabase</h1>
        <p style={{ marginTop: 4, fontSize: "0.875rem", color: "var(--mist)" }}>
          {evaluated.length > 0
            ? `เลขท้าย 2 ตัวตรงจริง ${back2Hits}/${evaluated.length} รายการที่ทราบผลแล้ว (${((back2Hits / evaluated.length) * 100).toFixed(1)}% — โอกาสสุ่มล้วนคือ ~1%)`
            : "ยังไม่มีคำทำนายที่ทราบผลจริงแล้ว"}
        </p>
      </div>
      {loading ? (
        <Card><CardContent style={{ padding: 40 }}><p style={{ fontSize: "0.85rem", color: "var(--mist)", textAlign: "center" }}>กำลังโหลด...</p></CardContent></Card>
      ) : rows.length === 0 ? (
        <Card><CardContent className="flex flex-col items-center text-center" style={{ padding: "48px 20px", gap: 12 }}>
          ...
          <p style={{ fontSize: "0.875rem", color: "var(--parchment)" }}>ยังไม่มีคำทำนายในฐานข้อมูล</p>
          <GoldButton onClick={() => setView("generate")}><Sparkles size={16} /> ไปสร้างคำทำนาย</GoldButton>
        </CardContent></Card>
      ) : (
        <Card><div className="ck-divide">
          {rows.map((p) => (
            <div key={`${p.target_draw_date}-${p.rank}-${p.source}`} className="flex flex-wrap items-center justify-between" style={{ padding: "16px 20px", gap: 12 }}>
              <div className="flex items-center" style={{ gap: 12 }}>
                <NumberRow digits={p.back2} size="sm" tone={p.evaluation ? (p.evaluation.back2_exact_hit ? "gold" : "mist") : "cold"} />
                <div>
                  <p style={{ fontSize: "0.75rem", color: "var(--parchment)" }}>งวด {formatThaiDate(p.target_draw_date)}</p>
                  <p style={{ fontSize: "0.68rem", color: "var(--mist)" }}>
                    {p.contributing_strategies?.length === 1 ? STRATEGIES.find((s) => s.id === p.contributing_strategies[0])?.nameTh ?? "แบบจำลองรวม" : "แบบจำลองรวม"} · อันดับ {p.rank}
                    {p.evaluation && ` · ผลจริง ${p.evaluation.actual_back2}`}
                  </p>
                </div>
              </div>
              <div className="flex items-center" style={{ gap: 8 }}>
                <Badge tone="gold">ความสอดคล้อง {p.agreement_score}%</Badge>
                {p.evaluation ? (
                  <Badge tone={p.evaluation.back2_exact_hit ? "gold" : "mist"}>{p.evaluation.back2_exact_hit ? "ตรง" : "ไม่ตรง"}</Badge>
                ) : (
                  <Badge tone="cold">รอผล</Badge>
                )}
              </div>
            </div>
          ))}
