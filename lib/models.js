...
export function digitTransitionMatrix(draws, field) {
  ...
  const sequence = field === "back2" ? sorted.map((d) => d.back2)
    : field === "firstPrizeA" ? sorted.map((d) => d.firstPrize)
    : field === "front3a" ? sorted.map((d) => d.front3[0]).filter(Boolean) : sorted.map((d) => d.back3[0]).filter(Boolean);
...
export function developmentDraws(allDraws, frozenBoundaryDate) {
  return allDraws.filter((d) => d.drawDate <= frozenBoundaryDate);
}
export function frozenDraws(allDraws, frozenBoundaryDate) {
  return allDraws.filter((d) => d.drawDate > frozenBoundaryDate);
}
export function assertNoFrozenLeakage(draws, frozenBoundaryDate, callerLabel) {
  const leaked = draws.filter((d) => d.drawDate > frozenBoundaryDate);
  if (leaked.length > 0) {
    throw new Error(
      `[FROZEN BOUNDARY VIOLATION in ${callerLabel}] ${leaked.length} draw(s) at/after ${frozenBoundaryDate} present ` +
      `(earliest: ${leaked[0].drawDate}). This function may only receive development/validation draws.`
    );
  }
}
...
const PICK_FIELD_FOR_TYPE = { firstPrize: "first", front3: "front3", back3: "back3", back2: "back2" };

// Digit-position ensemble variant of buildCandidates -- parallel function,
// buildCandidates above is untouched and keeps powering current production.
// weightsByPosition shape: { firstPrize: [posArr x6], front3: [posArr x3],
// back3: [posArr x3], back2: [posArr x2] }, each posArr = the array
// deployPositionWeightsFromState returns for that (type, position).
export function buildCandidatesWithPositionWeights(draws, weightsByPosition, count = 3) {
  const rng = makeRng(seedFromDraws(draws));
  const picks = computeStrategyPicks(draws, rng);
  const equalShare = 1 / STRATEGIES.length;

  function digitVotes(predictionType, slot, pos) {
    const arr = weightsByPosition[predictionType]?.[pos];
    const weightMap = new Map((arr || []).map((w) => [w.strategy, w.weight]));
    const pickField = PICK_FIELD_FOR_TYPE[predictionType];
    const tally = new Array(10).fill(0);
    for (const { strategy, pick } of picks) {
      const w = weightMap.get(strategy) ?? equalShare;
      const str = slot == null ? pick[pickField] : pick[pickField][slot];
      const d = Number(str?.[pos]);
      if (!Number.isNaN(d)) tally[d] += w;
    }
    return tally.map((weight, digit) => ({ digit, weight })).sort((a, b) => b.weight - a.weight);
  }
  function ensembleField(predictionType, slot, length) {
    const perPosition = Array.from({ length }, (_, pos) => digitVotes(predictionType, slot, pos));
    return { value: perPosition.map((v) => v[0].digit).join(""), perPosition };
  }
  ...
  const back3a = ensembleField("back3", 0, 3), back3b = ensembleField("back3", 1, 3);
  const front3a = ensembleField("front3", 0, 3), front3b = ensembleField("front3", 1, 3);
  const first = ensembleField("firstPrize", null, 6);
  ...
  for (let rank = 1; rank <= count; rank++) {
    ...
    // Agreement score is an unweighted share of strategies here (not
    // weighted by strategy) -- weight now varies by position, so there is
    // no single per-strategy weight left to attribute a whole-value pick
    // to. Deliberate simplification, not an oversight.
    const agreeing = picks.filter((p) => p.pick.back2 === back2Value).map((p) => p.strategy);
    const agreementScore = Math.round((agreeing.length / STRATEGIES.length) * 100);
    const avgConsensus = Math.round(
      (back2.perPosition.reduce((a, v) => a + v[0].weight, 0) / back2.perPosition.length) * 100
    );
    candidates.push({
      rank,
      firstPrize: first.value,
      ...
      contributingStrategies: agreeing.length ? agreeing : STRATEGIES.map((s) => s.id),
      explanationTh: rank === 1
        ? "รวมน้ำหนักจากทุกแบบจำลองตามผลการเรียนรู้แยกรายตำแหน่งของแต่ละประเภทเลข เลือกหลักที่ได้น้ำหนักโหวตสูงสุดในแต่ละตำแหน่ง"
        : "ตัวแปรรอง จากตำแหน่งที่แบบจำลองต่าง ๆ มีความเห็นก้ำกึ่งที่สุด",
    });
  }
  return candidates;
...
export function back2ToIndex(back2) { return Number(back2[0]) * 10 + Number(back2[1]); }
...
export function runFeatureDiscovery(draws, trainFraction = 0.7, frozenBoundaryDate = null) {
  if (frozenBoundaryDate) assertNoFrozenLeakage(draws, frozenBoundaryDate, "runFeatureDiscovery");
  const sorted = [...draws].sort((a, b) => a.drawDate.localeCompare(b.drawDate));
...
export function runEvolutionEngine(draws, opts = {}) {
  if (opts.frozenBoundaryDate) assertNoFrozenLeakage(draws, opts.frozenBoundaryDate, "runEvolutionEngine");
...
export function runMLModelsEvaluation(draws, trainFraction = 0.7, frozenBoundaryDate = null) {
  if (frozenBoundaryDate) assertNoFrozenLeakage(draws, frozenBoundaryDate, "runMLModelsEvaluation");
  const sorted = [...draws].sort((a, b) => a.drawDate.localeCompare(b.drawDate));
  const splitIdx = Math.floor(sorted.length * trainFraction);
  const rng = makeRng(seedFromDraws(sorted));
...
}function fieldDigitLength(field) { return field === "back2" ? 2 : field === "firstPrize" ? 6 : 3; }
function digitGapByPosition(pool, len) {
  // Per-position analogue of hotColdNumbers' "gap" signal, without
  // enumerating the whole-value universe (10^len is infeasible at len=6
  // for firstPrize -- hotColdNumbers' own approach only works for
  // back2/front3/back3, where the universe is <=1000).
  const gapByPos = Array.from({ length: len }, () => new Array(10).fill(pool.length));
  for (let p = 0; p < len; p++) {
    for (let d = 0; d < 10; d++) {
      for (let i = pool.length - 1; i >= 0; i--) {
        if (Number(pool[i][p]) === d) { gapByPos[p][d] = pool.length - 1 - i; break; }
      }
    }
  }
  return gapByPos.map((row) => {
    const total = row.reduce((a, b) => a + b, 0) || 1;
    return row.map((v) => v / total);
  });
}
function marginalizeJoint(jointDist, len) {
  // Collapses a full 10^len joint distribution down to `len` independent
  // per-position 10-way marginals. Only used for back2/front3/back3 below,
  // where the joint (<=1000 entries) is cheap to build in the first place.
  const marginals = Array.from({ length: len }, () => new Array(10).fill(0));
  for (let i = 0; i < jointDist.length; i++) {
    let rem = i;
    const digits = [];
    for (let p = 0; p < len; p++) { digits.unshift(rem % 10); rem = Math.floor(rem / 10); }
    for (let p = 0; p < len; p++) marginals[p][digits[p]] += jointDist[i];
  }
  return marginals;
}
...
export function strategyFieldPositionProbs(id, draws, field) {
  const len = fieldDigitLength(field);
  if (field !== "firstPrize") {
    return marginalizeJoint(strategyFieldFullDistribution(id, draws, field), len);
  }
  const pool = fieldPool(draws, field);
  if (pool.length === 0) return Array.from({ length: len }, () => new Array(10).fill(0.1));
  if (id === "entropy") return entropyWeights(pool).map((row) => laplaceRow(row));
  if (id === "pairCorrelation") {
    const posTable = positionFrequency(pool);
    const conditional = buildConditionalTable(pool, 1);
    const marginals = [laplaceRow(posTable[0])];
    for (let p = 1; p < len; p++) {
      const next = new Array(10).fill(0);
      for (let dPrev = 0; dPrev < 10; dPrev++) {
        const condRow = laplaceRow(conditional[p]?.get(String(dPrev)));
        const wPrev = marginals[p - 1][dPrev];
        for (let d = 0; d < 10; d++) next[d] += wPrev * condRow[d];
      }
      const total = next.reduce((a, b) => a + b, 0) || 1;
      marginals.push(next.map((v) => v / total));
    }
    return marginals;
  }
  if (id === "frequency" || id === "monteCarlo" || id === "bayesian" || id === "markov" || id === "gap" || id === "digitMomentum") {
    return strategyFieldDigitProbs(id, draws, field);
  }
    // tripletCorrelation/digitalRoot/mirrorNumber: whole-value signals with
  // no clean position decomposition; fall back to plain per-position
  // frequency for firstPrize specifically. Labeled simplification, not a bug.
  const table = positionFrequency(pool);
  return table.map((row) => laplaceRow(row));
}

// Field-generalized counterpart of strategyBack2DigitProbs (which stays
// untouched for the legacy back2 path). Module-local: both existing callers
// -- strategyFieldPositionProbs above and strategyFieldFullDistribution
// below -- are in this file. Body is the previously orphaned block,
// restored verbatim plus the frequency/monteCarlo branch it was missing.
function strategyFieldDigitProbs(id, draws, field) {
  const pool = fieldPool(draws, field);
  const len = fieldDigitLength(field);
  if (pool.length === 0) return Array.from({ length: len }, () => new Array(10).fill(0.1));
  if (id === "frequency" || id === "monteCarlo") {
    return positionFrequency(pool).map((row) => laplaceRow(row));
  }
  if (id === "bayesian") {
  ...
  if (id === "markov") {
    const matchField = field === "back2" ? "back2" : field === "firstPrize" ? "firstPrizeA" : field === "front3" ? "front3a" : "back3a";
  ...
  if (id === "gap") {
    if (field === "firstPrize") return digitGapByPosition(pool, len);
