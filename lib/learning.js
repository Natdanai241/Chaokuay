...
export function positionKey(strategyId, predictionType, position) {
  return `${strategyId}|${predictionType}|${position}`;
}
export function positionStateRowToState(row) {
  return {
    strategyId: row.strategy_id,
    evaluatedCount: row.evaluated_count,
    longTermMeanBrier: row.long_term_mean_brier,
...
export function positionStateToRow(key, state) {
