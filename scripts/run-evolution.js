...
import { runEvolutionEngine, STRATEGIES, precomputeStrategyDistributions, fitnessForWeights, MIN_WARMUP, developmentDraws } from "../lib/models.js";
...
async function fetchFrozenBoundary() {
  ...
  if (!rows[0]) throw new Error("No row in dataset_splits -- a frozen boundary must exist before Evolution Engine can run.");
...
async function main() {
  ...
  console.log(`[run-evolution] Loaded ${allDraws.length} total draws; ${draws.length} within development/validation boundary (frozen boundary: ${frozenBoundaryDate})`);
  console.log(`[run-evolution] Frozen draws excluded from this run: ${allDraws.length - draws.length}`);
  console.log(`[run-evolution] ${STRATEGIES.length} strategies in the pool`);

  const result = runEvolutionEngine(draws, { frozenBoundaryDate });
