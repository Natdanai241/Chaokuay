...
async function main() {
  if (!SUPABASE_URL || !SUPABASE_SECRET_KEY) {
    throw new Error(
      `Missing Supabase env vars -- SUPABASE_URL=${SUPABASE_URL ? "set" : "MISSING"}, SUPABASE_SECRET_KEY=${SUPABASE_SECRET_KEY ? "set" : "MISSING"}. ` +
      `Check repo Settings > Secrets and variables > Actions -- names must match exactly, and if they're Environment secrets the job needs an environment: key.`
    );
  }
