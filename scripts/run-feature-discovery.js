...
import { runFeatureDiscovery, FEATURES } from "../lib/models.js";
...
async function main() {
  ...
  console.log(`[run-feature-discovery] Loaded ${draws.length} draws`);
  console.log(`[run-feature-discovery] Feature pool: ${FEATURES.map((f) => f.id).join(", ")}`);

  const result = runFeatureDiscovery(draws);
