...
async function fetchLiveEvaluations() {
  const rows = [];
  const pageSize = 1000;
  for (let offset = 0; ; offset += pageSize) {
    const r = await fetch(
      `${SUPABASE_URL}/rest/v1/strategy_prediction_evaluations?select=strategy_id,back2_match_pct&order=evaluated_at.asc&offset=${offset}&limit=${pageSize}`,
      { headers: { apikey: SUPABASE_SECRET_KEY, Authorization: `Bearer ${SUPABASE_SECRET_KEY}` } }
    );
    if (!r.ok) throw new Error(`Fetch live evaluations failed: ${r.status} ${await r.text()}`);
    const page = await r.json();
    rows.push(...page);
    if (page.length < pageSize) break;
  }
...
async function main() {
  ...
  const liveEvaluations = await fetchLiveEvaluations();
  const finalWeights = computeLiveAdjustedWeights(predictionWeights, liveEvaluations);
  for (const w of finalWeights) console.log(`[run-backtest] Live adjustment -- ${w.strategy}: ${w.liveNote}`);
