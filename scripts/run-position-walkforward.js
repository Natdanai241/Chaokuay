...
// -- position_walkforward_summary support: same idea as emptyArmAccumulator/
// recordArm above, but keyed per (type, position) instead of collapsed
// across a whole type, since the DB table's unique key is
// (run_id, prediction_type, digit_position, variant). brierScore/
// rankOfActual are nullable -- arm A (buildCandidates) has no probability
// distribution, so it only ever contributes a hit/miss, never Brier/rank.
    ...
    if (legacyCandidate) {
      ...
      for (const type of Object.keys(PREDICTION_TYPE_POSITIONS)) {
        ...
        for (let pos = 0; pos < PREDICTION_TYPE_POSITIONS[type]; pos++) {
          ...
          recordPositionCell(positionSummaryA[type][pos], null, null, isHit);
        }
      }
    }
...
async function main() {
  ...
  // Always recorded, independent of COMMIT_POSITION_STATE -- this is the
  // A/B/C/D comparison result itself (read-only relative to learning state),
  // not a change to production weights, so there's no reason to gate it
  // behind the same dry-run flag.
